...
class TestLLMDecisions:
    ...
    async def test_fallback_publishes_reasoning_when_circuit_open(self):
        ...
        """
        player, fn, _, circuit = self._make_archetypes_and_fn(GUNSLINGER)
        circuit._open.add("openai/gpt-5-nano")  # pre-trip circuit
        gs = _make_game_state()
        gs.players = [player]

        with patch("app.ai.decision.acompletion") as mock_acomp,              patch("app.ai.decision.publish_reasoning", new_callable=AsyncMock) as mock_pub,              patch("app.ai.decision.calculate_equity", return_value={"win_probability": 0.5, "hand_strength": None, "pot_odds": None}):
...
```
  ...
  <acceptance_criteria>
    ...
    - test_fallback_publishes_reasoning_when_circuit_open PASSED (D-12 coverage)
...
<success_criteria>
- test_llm_decisions.py: 13+ tests passing (all classes, includes D-12 fallback publish test)
